...
\begin{align*}
  \sprod{\Gamma^k_{ij}f_k, f_l} &= \sprod{f_{ij}^\T, f_l} \\
  &= \sprod{f_{ij}, f_{l}}
...
Wir zerlegen nun
\begin{align*}
  X(\d f\cdot Y) = \left( X(\d f\cdot Y) \right)^\T + \left( X(\d f \cdot Y)
  \right)^\perp
\end{align*}
also $\left( X_p(\d f\cdot Y) \right)^\T, \left( X_p(\d f\cdot Y)
\right)^\perp\in N_pf$.
