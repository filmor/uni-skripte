...
\begin{lem}
  ...
  \begin{krl}
    ...
    sei $X^\intercal(p)$ die tangentiale und $X^\perp(p)$ die normale Komponente
    von $X(p)$, also $X^\intercal(p)\in T_pf$ und $X^\perp(p)\in N_pf$. Dann
    sind $X^\intercal,X^\perp: U\to\R^n$ glatt.

    \begin{proof}
      Wähle $U$ und $X_1,\dots,X_n$ wie im Lemma. Dann ist
      $X^\intercal=\sum_{i=1}^m\sprod{X,X_i}X_i$ und $X^\perp = X- X^\intercal$      
    \end{proof}
